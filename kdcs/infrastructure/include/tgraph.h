#ifndef KDELTA_TRUSS_TGRAPH_H_
#define KDELTA_TRUSS_TGRAPH_H_
...
class KDeltaTruss; // forward declaration
class MbaF; // forward declaration
class TGraph final{
  friend class KDeltaTrussCommunity::KDeltaTruss;
  friend class KDeltaTrussCommunity::MbaF;
  public:
  TGraph(const uint32_t n, const uint32_t l, const std::vector<uint32_t>& k)
      : l_(l), n_(n), m_(0), k_(k) {
  ...
  void ForEachTriangle(const uint32_t eid, Fn&& fn) const {
    ASSERT_MSG(UINT32_MAX != edge_info_.at(eid).first, "invalid edge ID");
    const uint32_t v1 = edge_info_[eid].first;
    const uint32_t v2 = edge_info_[eid].second;
    ASSERT(v1 < n_ && v2 < n_);
    ...
    while (p1 < adj_[v1].size() && p2 < adj_[v2].size()) {
      if (adj_[v1][p1].vid == adj_[v2][p2].vid) {
        fn(adj_[v1][p1].eid, adj_[v2][p2].eid);
        ++p1;
        ++p2;
      } else if (adj_[v1][p1].vid < adj_[v2][p2].vid) {
        ++p1;
  ...
  template <typename Fn>
  void ForEachTriangle(const uint32_t eid, const uint32_t k, Fn&& fn) const {
    ForEachTriangle(eid, [&](uint32_t e1, uint32_t e2) {
      if (k_[e1] >= k && k_[e2] >= k) fn(e1, e2);
    });
  }
  ...
  // insert an edge (v1, v2) and return its edge ID
  uint32_t InsertEdge(uint32_t v1, uint32_t v2, bool& type) {
    ASSERT_MSG(v1 < n_ && v2 < n_, "invalid insertion");
    ASSERT_MSG(m_ + 1 <= l_, "# of edges exceeded");
    // the ID of (v1, v2)
    // insert the edge to the adjacency arrays
  ...
  //get the minimum time span of the triangle 
  uint32_t GetMts(const std::vector<uint32_t> &times1,const std::vector<uint32_t> &times2,const std::vector<uint32_t> &times3){
    uint32_t i = 0, j = 0, k = 0;
    uint32_t n = times1.size(), m = times2.size(), p = times3.size();
    uint32_t delta_min = UINT32_MAX;
    while(i < n && j < m && k < p){
