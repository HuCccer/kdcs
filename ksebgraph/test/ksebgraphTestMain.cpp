...
int main(int argc, char** argv) {
    ...
    if (mode == "build") {
        ...
        MbaG mbaG(n, m, graph_file);
         if (stoi(op) == 0) {
            ...
            mbaG.writeTGraph("./" + base);
            mbaG.writeKSEBGraphsIndexToFile("./ksebgraph_" + base);
        } else {
            auto beg = std::chrono::steady_clock::now();
            mbaG.onePass();
            auto end = std::chrono::steady_clock::now();
    ...
    /* ================= QUERY ================= */
        ...
        infile.close();
        MbaG mbaG(n, m * 2);
        mbaG.readTGraphFromFile(graph_file);
        mbaG.readKSEBGraphsIndexFromFile(kseb);
        ...
        for (int i = 0; i < query_vertices.size(); i++) {
            mbaG.searchCommunityByKSEBraphs(query_vertices[i], k, delta);
        }
        auto end = std::chrono::steady_clock::now();
